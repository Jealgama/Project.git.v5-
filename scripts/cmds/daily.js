...
module.exports = {
	...
	onStart: async function ({ args, message, event, envCommands, usersData, commandName, getLang }) {
		...
		if (args[0] == "info") {
			let msg = "";
			for (let i = 1; i < 8; i++) {
				const getCoin = Math.floor(reward.coin * (1 + 20 / 28000000) ** ((i == 0 ? 7 : i) - 1));
				const getExp = Math.floor(reward.exp * (1 + 20 / 28000000) ** ((i == 0 ? 7 : i) - 1));
		...
		const getCoin = Math.floor(reward.coin * (1 + 20 / 28000000) ** ((currentDay == 0 ? 7 : currentDay) - 1));
		const getExp = Math.floor(reward.exp * (1 + 20 / 28000000) ** ((currentDay == 0 ? 7 : currentDay) - 1));
